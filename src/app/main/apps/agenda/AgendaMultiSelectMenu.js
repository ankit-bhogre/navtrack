...
import { setContactsUnstarred, setContactsStarred, removeAgendas } from './store/agendaSlice';

function AgendaMultiSelectMenu(props) {
	const dispatch = useDispatch();
	const { selectedContactIds } = props;

	const [anchorEl, setAnchorEl] = useState(null);

	function openSelectedAgendaMenu(event) {
	...
	return (
		<>
			<IconButton
				...
				aria-haspopup="true"
				onClick={openSelectedAgendaMenu}
			>
			...
			>
				<MenuList>
					<MenuItem
						onClick={() => {
							dispatch(removeAgendas(selectedContactIds));
...
export default AgendaMultiSelectMenu;
